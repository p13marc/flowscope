...
**Question:** "Do we have to improve or add features to flowscope first?"

**Short answer:** Yes. Six concrete flowscope work items — three required for the zero-allocation contract, one required for the architectural design, two highly desirable — should land in **flowscope 0.11** before netring 0.19 implementation starts. Estimated effort: **~8–12 working days** on the flowscope side. Without them, netring 0.19 either ships with known allocations on the hot path (violating its load-bearing perf claim) or ships with workarounds it later has to rip out.

Below is the full audit: what netring 0.19 needs, what flowscope 0.10 already provides, what's missing, and a recommended sequencing.
