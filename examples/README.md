...
| **`http2_driver`** *(0.23)* | `http2`, `session` | h2 on the typed `Driver` via `Http2Session` (#196) — the counterpart to driving `Http2Parser` yourself. Shows why `stream_id`, not `side`, is the routing key, and that a flow picked up mid-connection parses instead of poisoning. |
| **`http2_header_rewrite`** *(0.23)* | `http2` | Rewrite an h2 request header and put it back on the wire with `HpackEncoder` + `write_headers` (#197). The thing HTTP/1 gets free: h2 headers are a stateful compressed encoding, so any change means re-encoding. Shows the table compounding, why `authorization` is never indexed, and the `CONTINUATION` split. |
