...
fn bench_passthrough(c: &mut Criterion) {
    ...
    group.bench_function("passthrough", |b| {
        let mut d =
            FlowSessionDriver::<_, NoopParser>::new(FiveTuple::bidirectional());
        // 3WHS first so the flow is established before benchmarking.
        let mac = [0u8; 6];
        let syn = ipv4_tcp(mac, mac, [10, 0, 0, 1], [10, 0, 0, 2], 1234, 80, 1000, 0, 0x02, b"");
        let synack = ipv4_tcp(mac, mac, [10, 0, 0, 2], [10, 0, 0, 1], 80, 1234, 5000, 1001, 0x12, b"");
        let ack = ipv4_tcp(mac, mac, [10, 0, 0, 1], [10, 0, 0, 2], 1234, 80, 1001, 5001, 0x10, b"");
        for f in &[syn, synack, ack] {
            d.track(PacketView::new(f, Timestamp::default()));
        }
        let payload = vec![b'A'; 1400];
        let data = ipv4_tcp(
            mac, mac, [10, 0, 0, 1], [10, 0, 0, 2], 1234, 80, 1001, 5001, 0x18, &payload,
        );
        // Benchmark: feed a data segment in a loop, advancing seq.
        let mut seq = 1001u32;
        b.iter(|| {
            let frame = ipv4_tcp(
                mac, mac, [10, 0, 0, 1], [10, 0, 0, 2], 1234, 80, seq, 5001, 0x18, &payload,
            );
